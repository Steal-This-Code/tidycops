...
`tidycops` is an incident-focused R package for pulling public police data across cities with one interface.
...
Use `last_n_days` when you want a trailing window without manual date math.
...
Some adapters are rolling windows, capped historical feeds, or scope-limited
(for example calls-for-service or Part I only). Check source metadata before
analysis.
...
Before expanding to arrests/UOF, these are the highest-value additions for
incident workflows:
